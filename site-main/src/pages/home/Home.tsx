...
export default function Home() {
  ...
  useEffect(() => {
    api.get("/api/ocorrencias").then(res => setOcorrencias(res.data.length)).catch(() => {});
    api.get("/api/relatorios").then(res => setRelatorios(res.data.length)).catch(() => {});
    if (user?.role === "ADMIN") {
      api.get("/usuarios").then(res => setUsuarios(res.data.length)).catch(() => {});
    }
  }, [user]);
